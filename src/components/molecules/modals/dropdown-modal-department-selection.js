...
const DropdownModalDepartmentTypeSelection = ({
  style = {},
  value,
  visible = false,
  onClose = item => {},
          ...
          {items?.map((item, index) => {
            return (
              <TouchableOpacity
                key={index}
                onPress={() => {
                  onChangeText(item?.DepartmentId); // Pass only the Id
                  onClose(); // Close the modal
                }}
                style={styles.button}>
                <Medium
                  color={colors.primary}
                  label={item?.title || item?.name || item?.DepartmentName || item?.DepartmentId}
...
export default DropdownModalDepartmentTypeSelection;
